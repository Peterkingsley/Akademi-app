...
export const EPISODE_TEACHING_ANALYSIS_RESPONSE_SCHEMA = {
  ...
  properties: {
    ...
    episode_thesis: {
      type: 'object',
      properties: { statement: string, claim_type: string, evidence_ids: evidenceIdList, epistemic_status: string },
    ...
    concepts: {
      ...
      items: {
        type: 'object',
        properties: {
          concept_id: string, canonical_name: string, tier: string, teaching_priority: { type: 'number' }, epistemic_status: string,
          prerequisite_concept_ids: stringList, evidence_ids: evidenceIdList, selected_analogy_id: { type: 'string', nullable: true },
          invariants: {
            type: 'array', items: {
              type: 'object', properties: { invariant_id: string, statement: string, claim_type: string, evidence_ids: evidenceIdList, epistemic_status: string, forbidden_exaggerations: stringList },
              required: ['invariant_id', 'statement', 'claim_type', 'evidence_ids', 'epistemic_status', 'forbidden_exaggerations'],
            },
          },
          causal_relationships: { type: 'array', items: { type: 'object', properties: { relationship_type: string, statement: string, evidence_ids: evidenceIdList }, required: ['relationship_type', 'statement', 'evidence_ids'] } },
          misconceptions: { type: 'array', items: { type: 'object', properties: { misconception_id: string, naive_assumption: string, why_it_feels_intuitive: string, correction_target: string, severity: string, must_surface_in_dialogue: { type: 'boolean' } }, required: ['misconception_id', 'naive_assumption', 'why_it_feels_intuitive', 'correction_target', 'severity', 'must_surface_in_dialogue'] } },
