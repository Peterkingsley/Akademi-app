...
const stringArray = (value: unknown) => Array.isArray(value) && value.every((item) => nonEmpty(item));
...
export function validateAnalysis(value: unknown, analogyRiskThreshold = 0.35): EpisodeTeachingAnalysis {
  ...
  for (const concept of analysis.concepts) {
    ...
    if (!stringArray(concept.evidence_ids) || concept.evidence_ids.some((id) => !evidenceIds.has(id))) issues.push(`Concept ${concept.concept_id} references missing evidence.`);

    for (const invariant of concept.invariants) {
      if (!isRecord(invariant) || !nonEmpty(invariant.invariant_id) || !nonEmpty(invariant.statement) || !stringArray(invariant.evidence_ids) || !Array.isArray(invariant.forbidden_exaggerations)) {
  ...
  if (!stringArray(analysis.episode_thesis.evidence_ids) || analysis.episode_thesis.evidence_ids.some((id) => !evidenceIds.has(id))) issues.push('Episode thesis references missing evidence.');
...
export function validateBlueprint(value: unknown, analysis: EpisodeTeachingAnalysis): EpisodeTeachingBlueprint {
  ...
  for (const turn of blueprint.turns) {
    ...
    for (const id of turn.concept_ids || []) { if (!conceptIds.has(id)) issues.push(`Blueprint references unknown concept ${id}.`); else representedConcepts.add(id); }
    for (const id of turn.invariant_ids || []) if (!invariantIds.has(id)) issues.push(`Blueprint references unknown invariant ${id}.`);
    for (const id of turn.misconception_ids || []) { if (!misconceptionIds.has(id)) issues.push(`Blueprint references unknown misconception ${id}.`); else handledMisconceptions.add(id); }
    for (const id of turn.evidence_ids || []) if (!evidenceIds.has(id)) issues.push(`Blueprint references unknown evidence ${id}.`);
  }
