...
const qualificationClause = /\b(?:do(?:es)?n't\s+(?:make|eliminate|guarantee)|do\s+not\s+(?:make|eliminate|guarantee)|not\s+guaranteed|cannot\s+guarantee|can't\s+guarantee|rather\s+than\s+guarantee(?:ing)?|rare\s+but\s+possible|less\s+likely\s*,?\s*(?:but|not)\s+impossible|still\s+possible|can\s+still\s+happen|not\s+a\s+silver\s+bullet)\b/i;
...
const recoveryClause = /\b(?:retry|try again|repeated\s+election\s+attempts?|new\s+election\s+(?:attempt|round|gives?|offers?|provides?|allows?)|another\s+chance|recover|recovery|next\s+(?:term|election)\s+(?:gives?|offers?|provides?|allows?|brings?)|another\s+round)\b/i;
...
const causalConsequenceClause = /\b(?:make|makes|making|reduce|reduces|reducing|lower|lowers|lowering|prevent|prevents|preventing|avoid|avoids|avoiding)\s+(?:the\s+)?(?:(?:chance|chances|likelihood|frequency)\s+(?:of\s+)?)?(?:split\s+votes?|election\s+contention|leadership\s+contention|failed\s+elections?)\b/i;
