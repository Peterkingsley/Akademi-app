...
type SvgWhiteboardRendererProps = {
...
export const SvgWhiteboardRenderer: React.FC<SvgWhiteboardRendererProps> = ({
  plan,
  autoPlay = true,
  width,
  height = 360,
  ...
  useEffect(() => {
    ...
    const buildRenderedAction = (action: WhiteboardAction, previousY: number): RenderedAction | null => {
      ...
      if (action.type === "draw_line" || action.type === "draw_arrow") {
        const from = action.from || { x: 36, y: y };
    ...
    const run = async () => {
      ...
      try {
        ...
        if (mounted) {
          setError("The SVG whiteboard could not finish rendering.");
        }
      }
    };
  ...
  }, [autoPlay, canvasHeight, onComplete, plan, replayKey]);
  ...
  if (!plan || !plan.steps?.length) {
    return (
      <View style={styles.emptyCard}>
        ...
        <Text style={styles.emptyText}>No whiteboard plan is loaded for the SVG renderer yet.</Text>
      </View>
    );
  }
    ...
    <View style={styles.container}>
      <View style={[styles.boardShell, width ? { width } : null, { height: canvasHeight }]}>
        <Svg width="100%" height="100%" viewBox={`0 0 ${canvasWidth} ${canvasHeight}`}>
          <Rect x={0} y={0} width={canvasWidth} height={canvasHeight} fill="#0E1B31" rx={18} />
          ...
          {renderedActions.map((action) => {
            ...
            if (action.type === "write_text" || action.type === "write_math") {
              ...
              const visibleChars = Math.max(1, Math.floor(fullText.length * progress));
              return (
                <SvgText
                  key={action.id}
                  x={action.x}
                  y={action.y}
                  fill={action.color}
                  fontSize={action.type === "write_math" ? 22 : 20}
                  fontWeight="600"
                  fontFamily="System"
                >
            ...
            if (action.type === "draw_line" || action.type === "draw_arrow") {
              ...
              return (
                <React.Fragment key={action.id}>
                  <Line x1={from.x} y1={from.y} x2={currentX} y2={currentY} stroke={action.color} strokeWidth={3} strokeLinecap="round" />
                  {action.type === "draw_arrow" && progress > 0.94 ? (
                    <>
                      <Line
                        x1={to.x}
                        y1={to.y}
                        x2={to.x - headLength * Math.cos(angle - Math.PI / 6)}
                        y2={to.y - headLength * Math.sin(angle - Math.PI / 6)}
                        stroke={action.color}
                        strokeWidth={3}
                        strokeLinecap="round"
                      />
            ...
            if (action.type === "cross_out") {
              return (
                <Line
                  ...
                  x1={action.x - 8}
                  y1={action.y - 8}
                  x2={action.x - 8 + (action.width + 16) * progress}
                  y2={action.y - 8 + (action.height + 16) * progress}
                  stroke={action.color}
                  strokeWidth={3}
                  strokeLinecap="round"
                />
      ...
      {error ? (
        <View style={styles.errorCard}>
          <Text style={styles.errorTitle}>SVG renderer fallback</Text>
          <Text style={styles.errorText}>{error}</Text>
        </View>
      ) : null}

      <View style={styles.legendCard}>
        <Text style={styles.legendTitle}>SVG renderer note</Text>
        <Text style={styles.legendText}>
          Math is shown with a lightweight readable formatter for now, for example {formatWhiteboardLatexForDisplay(String.raw`\frac{1}{\sqrt{x} + 2}`)}.
        </Text>
      </View>
    </View>
  );
